...
class KinematicsNode(Node):                                             # ROS 2의 Node 클래스를 상속받아 나만의 기구학 노드를 만듭니다.
    def __init__(self):
        ...
        # [기능 추가] Dynamixel 개별 초기 위치 (FL, FR, RL, RR 순서)
        # self.initial_positions = [1021.0, 1002.0, 2059.0, 3052.0]
        self.initial_positions = [1024, 2048, 2048, 3072]
...
# =======================================================================================================================================
    ...
    def vel_callback(self, msg):
        ...
        # 라디안 각도를 다이나믹셀 모터의 틱(Tick) 값으로 변환하는 내부 함수 (개별 초기위치 반영)
        def get_dxl(rad, idx):
            ...
            return float(int(round(self.initial_positions[idx] + offset)))                           # 개별 초기값에 더하고 반올림하여 틱 생성
...
# =======================================================================================================================================
        ...
        if w == 0:
            ...
            steering_angles = [float(pos) for pos in self.initial_positions]        # 모든 바퀴가 각자의 초기 위치를 바라보도록 설정합니다.
...
# =======================================================================================================================================
            ...
            if w > 0:
                ...
                steer_rr_rad = math.atan(y / ((2 * R) + x)) if (R + x / 2.0) != 0 else alpha_rad        # 전륜 우측: 회전 반경차를 고려하여 아크탄젠트로 내측 바퀴보다 작은 각도를 계산
                steer_fr_rad = -steer_rr_rad                                                            # 후륜 우측: 전륜 우측의 반대 방향 (역상)
...
# =======================================================================================================================================
                ...
                steer_fl_rad = math.atan(y / ((2 * R) + x)) if (R + x / 2.0) != 0 else alpha_rad        # 후륜 좌측: 애커만 수식을 적용하여 내측 바퀴보다 작은 각도를 계산
                steer_rl_rad = -steer_fl_rad                                                            # 전륜 좌측: 전륜 좌측의 반대 방향 (역상)
            ...
            rad_list = [steer_fl_rad, steer_fr_rad, steer_rl_rad, steer_rr_rad]
            steering_angles = [get_dxl(rad_list[i], i) for i in range(4)]
...
# =======================================================================================================================================
        ...
        # [수정 사항] 조향각 인덱스 교체 (1번↔2번, 3↔4번 값이 바뀌어 나오는 현상 수정)
        # 0(FL)과 1(FR) 교체, 2(RL)와 3(RR) 교체
        ...
        # [수정 사항] 휠 속도 인덱스 교체 (FL, FR 교체)
        wheel_speeds[0], wheel_speeds[1] = wheel_speeds[1], wheel_speeds[0]
        
        # wheel_speeds 반전 로직 유지
        wheel_speeds[1] = -wheel_speeds[1]  # 1번 바퀴 반전
        wheel_speeds[2] = -wheel_speeds[2]  # 3번 바퀴 반전
