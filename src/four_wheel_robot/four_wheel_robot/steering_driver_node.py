...
class SteeringDriverNode(Node):
    def __init__(self):
        ...
        # [기능 추가] 각 다이나믹셀별 개별 초기 위치(Center) 설정
        # ID 순서대로 [ID 1, ID 2, ID 3, ID 4]의 값을 입력하세요.
        ...
        # 모터 초기화 (토크 끄기 -> 모드 변경 -> 토크 켜기 -> 각자의 초기 위치 이동)
        for i, dxl_id in enumerate(self.dxl_ids):
            init_pos = self.initial_positions[i]
            self.init_dynamixel(dxl_id, init_pos)
        ...
        self.get_logger().info("Steering Driver Node is Ready with Individual Offsets")

    # =========================================================
    # 다이나믹셀 초기화 함수 (에러 체크 및 개별 초기위치 반영)
    # =========================================================
    def init_dynamixel(self, dxl_id, init_pos):
        try:
            # 1. Torque Off
            ...
            # 4. 각자 지정된 초기 위치 이동
            comm_result, error = self.packet_handler.write4ByteTxRx(self.port_handler, dxl_id, ADDR_GOAL_POSITION, init_pos)
            if comm_result != COMM_SUCCESS:
                ...
                self.get_logger().info(f"Dynamixel ID {dxl_id}: Extended Mode & 초기위치({init_pos}) 설정 완료")
    ...
    # =========================================================
    # 3초 Watchdog 체크 (개별 초기 위치 반영)
    # =========================================================
    def watchdog_check(self):
        time_diff = (self.get_clock().now() - self.last_msg_time).nanoseconds / 1e9
        # 3초 이상 데이터가 없을 경우 (기존 333.0 오타 수정)
        if time_diff > 3.0 and not self.watchdog_triggered:
            self.get_logger().error("[Watchdog] 3초간 데이터 없음: 개별 초기 위치로 복귀합니다.")
            for i, dxl_id in enumerate(self.dxl_ids):
                init_pos = self.initial_positions[i]
                self.packet_handler.write4ByteTxRx(self.port_handler, dxl_id, ADDR_GOAL_POSITION, init_pos)
    ...
    def destroy_node(self):
        self.get_logger().info("노드 종료: 각 모터를 개별 초기 위치로 이동 후 토크를 해제합니다.")
        for i, dxl_id in enumerate(self.dxl_ids):
            init_pos = self.initial_positions[i]
            # 초기 위치 이동
            self.packet_handler.write4ByteTxRx(self.port_handler, dxl_id, ADDR_GOAL_POSITION, init_pos)
